...
module.exports = () => {
    ...
    */
    ...
    route.get('/employeeinfo', (req, res) => {
        const selSql = "SELECT employee_id as '社員番号',name,name as '名前',frigana as 'フリガナ',DATE_FORMAT( entering_date , '%Y/%m/%d' ) '入社年月日' from employee ";
    ...
    route.get('/employees', (req, res) => {
        const selSql = "SELECT employee_id ,name,frigana ,DATE_FORMAT( entering_date , '%Y/%m/%d' ) entering_date from employee ";
        db.query(selSql, (err, data) => {
            if (err) {
                console.log(err);
                res.status(500).send('database err').end();
    ...
    route.get('/employeeCertifications', (req, res) => {
        const selSql = "SELECT e.employee_id ,name,frigana ,DATE_FORMAT( entering_date , '%Y/%m/%d' ) entering_date,"
    ...
    route.post('/postEmployeeCertifications', (req, res) => {
        ...
        if (getDate && getDate != ""){
            selSql = `${selSql} AND c.get_date = '${getDate}'`;
        }
        if (encourageDate && encourageDate != ""){
            selSql = `${selSql} AND c.encourage_date = '${encourageDate}'`;
        }
        db.query(selSql, (err, data) => {
            if (err) {
                console.log(err);
